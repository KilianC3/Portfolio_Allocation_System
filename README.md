...
4. Start the service which launches the API, scheduler and all scrapers
   ...
   for compatibility but the value is ignored. Composite fundamentals are
   generated by `scrapers/full_fundamentals.py` and trending tickers are ranked
   by `scrapers/wiki.py`.
5. Install the optional test dependencies and run the unit tests
   ...
   pytest -q
   ```
   See `docs/AGENTS_ROOT.md` and the new `AGENTS.md` at the project root for
   commit guidelines, repository layout and agent usage details.
6. The `scrapers/full_fundamentals.py` module computes composite fundamentals
   for the combined universe with rate‑limited Yahoo downloads. The
   updated `scrapers/wiki.py` script also includes a momentum‑based
   portfolio builder for ranking universe tickers. Both scrapers run
   automatically via APScheduler when the service starts.
...
| Fundamental Scores | produced via `full_fundamentals.py` |
