...
async def test_latency_p95_under_5ms():
    ...
    with respx.mock(base_url="https://paper-api.alpaca.markets") as m:
        m.get("/v2/account").mock(return_value=httpx.Response(200, json={"portfolio_value": 100000}))
        m.get(re.compile(r"/v2/stocks/.*/trades/latest")).mock(return_value=httpx.Response(200, json={"trade": {"p": 100}}))
