...
def test_compute_weights_caps_and_drift():
    ...
    w = compute_weights(df, w_prev=prev, lambda_fee=0.1, sector_map=sector_map, sector_caps=caps)
    assert abs(sum(w.values()) - 1) < 1e-6
    assert w["A"] + w["B"] <= 0.6 + 1e-6
    w2 = compute_weights(df, w_prev=w, drift_band=0.05, sector_map=sector_map, sector_caps=caps)
    assert w2 == w
